...
const PlaylistItem: React.FC<IPlaylist> = ({ audioControl }) => {
  const { playlistMenu, fetchPlaylistPending } = useAppSelector(({ playlist }) => {
    return {
      ...
      fetchPlaylistPending: playlist.fetchPlaylistPending,
    };
  });
  return (
          ...
          <div className='playlist-item__header'>
            <div className='playlist-item__wrapper'>
              <div className='playlist-item__image'>
                <img src={playlistMenu.images[0].url} alt='#'></img>
              </div>
              <div className='playlist-item__info info-playlist'>
                <div className='info-playlist__title'>PLAYLIST</div>
                <h3 className='info-playlist__name'>{playlistMenu.name}</h3>
                <div className='info-playlist__author'>
                  Created By: <span>{playlistMenu.owner.display_name}</span> -
                  {playlistMenu.tracks.limit} songs
                </div>
